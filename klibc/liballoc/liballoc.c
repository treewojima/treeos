...
#define ALIGN( ptr )													\
		...
		{																\
            ...
            _s_diff = (uintptr_t)ptr & (ALIGNMENT-1);						\
            if ( _s_diff != 0 )											\
			{															\
                _s_diff = ALIGNMENT - _s_diff;								\
                ptr = (void*)((uintptr_t)ptr + _s_diff);					\
			}															\
...
#define UNALIGN( ptr )													\
		if ( ALIGNMENT > 1 )											\
		{																\
            uintptr_t _s_diff = *((ALIGN_TYPE*)((uintptr_t)ptr - ALIGN_INFO));	\
            if ( _s_diff < (ALIGNMENT + ALIGN_INFO) )						\
			{															\
                ptr = (void*)((uintptr_t)ptr - _s_diff);					\
			}															\
		}
